...
// Piano azzurro (questa volta: MESH CLICCABILE)
const activePlaneZ = 50;
const planeMesh = new THREE.Mesh(
  new THREE.PlaneGeometry(GRID_SIZE, GRID_SIZE),
  new THREE.MeshBasicMaterial({ color: 0x2b6cb0, transparent: true, opacity: 0.22, side: THREE.DoubleSide })
);
...
function pickOnPlane(clientX, clientY) {
...
payBtn.addEventListener("click", async () => {
  ...
  alert(`Revolut will open.\nSend €${current.price} and paste the note (copied).`);
  window.open(REVOLUT_LINK, "_blank", "noopener,noreferrer");
  window.location.href = "./buy.html";
});

// Drag rectangle (visuale) + selezione
...
renderer.domElement.addEventListener("click", (e) => {
  if (dragging) return;
  const p = pickOnPlane(e.clientX, e.clientY);
  if (!p) return;
...
renderer.domElement.addEventListener("pointerup", (e) => {
  ...
  const a = pickOnPlane(dragStart.x, dragStart.y);
  const b = pickOnPlane(dragEnd.x, dragEnd.y);
